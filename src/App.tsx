...
function drawRoundedRect(context: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r: number) {
  context.beginPath()
...
const App = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    context.fillText(state.badge || '', state.layout === 'center' ? badgeX + badgeWidth / 2 : badgeX + badgePadX, badgeY + badgeHeight / 2 + 1)
    ...
    const lineHeight = Math.round(state.titleSize * 1.15)
    titleLines.forEach((line, index) => {
      context.fillText(line, contentX, titleY + index * lineHeight)
    })

    const subY = titleY + titleLines.length * lineHeight + 22
    ...
    if (state.layout === 'split') {
      ...
      context.fillText('POINT', boxX + 26, boxY + 28)

      const bullets = ['PNG保存できる', '背景画像も使える', 'X向けサイズを選べる']
      context.fillStyle = theme.text
  ...
  }, [bgImage, preset.height, preset.width, state, theme])
      ...
      >
        <Panel style={{ padding: 24, position: isMobile ? 'static' : 'sticky', top: 24 }}>
          ...
          <p style={{ margin: '0 0 18px', color: '#64748b', fontSize: '0.94rem', lineHeight: 1.6 }}>
            Gitで配布しやすいVite + React構成。タイトル・背景・レイアウトを調整して、PNG保存できます。
          </p>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, background: '#eef2f7', padding: 6, borderRadius: 18, marginBottom: 20 }}>
            <button style={tabButtonStyle(activeTab === 'text')} onClick={() => setActiveTab('text')}>文字</button>
            <button style={tabButtonStyle(activeTab === 'style')} onClick={() => setActiveTab('style')}>見た目</button>
            <button style={tabButtonStyle(activeTab === 'image')} onClick={() => setActiveTab('image')}>背景</button>
          </div>
          ...
          {activeTab === 'text' && (
            <div>
              <div style={{ marginBottom: 18 }}>
                ...
                <input style={baseInputStyle} value={state.badge} onChange={(e) => setField('badge', e.target.value)} />
              </div>
              <div style={{ marginBottom: 18 }}>
                <FieldLabel>タイトル</FieldLabel>
                <textarea style={{ ...baseInputStyle, minHeight: 110, resize: 'vertical', lineHeight: 1.6 }} value={state.title} onChange={(e) => setField('title', e.target.value)} />
              </div>
              <div style={{ marginBottom: 18 }}>
                <FieldLabel>サブタイトル</FieldLabel>
                <textarea style={{ ...baseInputStyle, minHeight: 110, resize: 'vertical', lineHeight: 1.6 }} value={state.subtitle} onChange={(e) => setField('subtitle', e.target.value)} />
              </div>
            </div>
          )}
          ...
          {activeTab === 'style' && (
            <div>
              <div style={{ marginBottom: 18 }}>
                ...
                <select style={baseInputStyle} value={state.preset} onChange={(e) => setField('preset', e.target.value as PresetKey)}>
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <select style={baseInputStyle} value={state.layout} onChange={(e) => setField('layout', e.target.value as LayoutKey)}>
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                          <span style={{ width: 16, height: 16, borderRadius: 999, display: 'block', background: currentTheme.bg[0] }} />
                          <span style={{ width: 16, height: 16, borderRadius: 999, display: 'block', background: currentTheme.bg[1] }} />
                          <span style={{ width: 16, height: 16, borderRadius: 999, display: 'block', background: currentTheme.accent }} />
                        </div>
              ...
              <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 20, padding: 14, marginTop: 14 }}>
                {[
                  ...
                  <div key={item.key} style={{ marginBottom: index === 3 ? 0 : 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 8, fontSize: 14, fontWeight: 700 }}>
          ...
          {activeTab === 'image' && (
            <div>
              <div style={{ marginBottom: 18 }}>
                ...
                <input ref={fileInputRef} style={baseInputStyle} type="file" accept="image/*" onChange={(e) => handleImageUpload(e.target.files?.[0] ?? null)} />
                <div style={{ marginTop: 6, color: '#64748b', fontSize: 13, lineHeight: 1.5 }}>画像を敷いた上からオーバーレイをかけます。</div>
              </div>
              <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 20, padding: 14, marginTop: 14 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 8, fontSize: 14, fontWeight: 700 }}>
                  ...
                  <input type="range" min={0} max={80} step={1} value={state.overlay} onChange={(e) => setField('overlay', Number(e.target.value))} style={{ width: '100%' }} />
                </div>
              </div>
              {bgImageUrl && (
                <div style={{ marginTop: 10, borderRadius: 18, overflow: 'hidden', border: '1px solid #d5dde8', background: '#fff' }}>
                  <img src={bgImageUrl} alt="アップロード画像プレビュー" style={{ width: '100%', height: 180, objectFit: 'cover', display: 'block' }} />
                </div>
              )}
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 20 }}>
            <button onClick={downloadPng} style={{ border: 0, borderRadius: 16, padding: '13px 18px', font: 'inherit', fontWeight: 800, cursor: 'pointer', background: '#0f172a', color: '#fff' }}>
              PNG保存
            </button>
            <button onClick={resetAll} style={{ border: 0, borderRadius: 16, padding: '13px 18px', font: 'inherit', fontWeight: 800, cursor: 'pointer', background: '#e2e8f0', color: '#0f172a' }}>
              リセット
            </button>
          </div>
        </Panel>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: viewportWidth <= 720 ? '1fr' : 'repeat(3, minmax(0, 1fr))', gap: 16 }}>
            {[
              ['レイアウト3種', '左寄せ・中央・分割'],
              ['カラーテーマ切替', '濃色・淡色をすぐ変更'],
          ...
          <Panel style={{ padding: 22 }}>
            ...
            <div style={{ overflow: 'auto', borderRadius: 24, background: 'linear-gradient(180deg, #dbe4ef 0%, #cfd9e6 100%)', padding: 18, minHeight: 420 }}>
              <canvas
                ref={canvasRef}
                style={{
                  display: 'block',
